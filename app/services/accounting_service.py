...
async def consume_agent_authorization_topup_points(
    *,
    agent_id: int,
    user_id: int,
    project_id: int,
    ...
    """旧设备补时并批扣点：按自定义金额扣点并生成授权扣点快照。"""
    if authorized_devices <= 0:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="补时设备数必须大于 0")
    if billing_period not in ("week", "month"):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="补时计费周期不合法")
    if billing_period_hours <= 0:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="补时计费小时数不合法")
    if paid_hours <= 0:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="补时小时数必须大于 0")
    ...
    snapshot = AuthorizationChargeSnapshot(
        ...
        billing_period_hours=int(billing_period_hours),
        period_count=1,
        paid_hours=int(paid_hours),
    ...
    description = (
        f"旧设备补时并批扣点：用户ID={user_id}，项目ID={project_id}，"
        f"{LEVEL_NAMES.get(user_level, user_level)}，{authorized_devices}台，"
        f"补时{paid_hours}小时，单价{_fmt_money(unit_price)}点/台，"
        f"统一到期{_fmt_dt(valid_until)}，合计{_fmt_money(total_cost_dec)}点"
    )
    ...
    if charged > 0 and remaining > 0:
        ...
        await _append_ledger_entry(
            ...
            balance_after=after,
            business_category="authorization_charge",
            business_subtype="batch_topup_align_charged",
            related_user_id=user_id,
            related_project_id=project_id,
            related_authorization_id=authorization_id,
            related_charge_snapshot_id=snapshot.id,
            description=description,
            idempotency_key=f"authorization:{authorization_id}:{snapshot.id}:topup_charged",
            source="agent",
            operated_by_agent_id=agent_id,
        )
    ...
    if remaining > 0:
        ...
        await _append_ledger_entry(
            ...
            balance_after=after,
            business_category="authorization_charge",
            business_subtype="batch_topup_align_credit",
            related_user_id=user_id,
            related_project_id=project_id,
            related_authorization_id=authorization_id,
            related_charge_snapshot_id=snapshot.id,
            description=description,
            idempotency_key=f"authorization:{authorization_id}:{snapshot.id}:topup_credit",
            source="agent",
            operated_by_agent_id=agent_id,
        )
